...
const {lotteryConfig, lottery,getTodayStatus } = require('../api');
...
const drawFn = async () => {
  ...
  if(today.err_no) return Promise.reject('已经签到！今日已经免费抽奖！')
  if(!today.data.free_count) return Promise.resolve('签到成功！今日已经免费抽奖！')

  // 免费抽奖
  const draw = await lottery(headers)
  if (draw.err_no) return Promise.reject('已经签到！免费抽奖异常！');
